...
export class News extends Component {
    ...
    async componentDidMount() {
        ...
        if (searchTerm.trim() !== '') {
            try {
                // let response = await fetch(`https://newsapi.org/v2/everything?q=${searchTerm}&apiKey=d20393bf939f47f9b0b06485bdbf6d76`);
                // if (!response.ok) {
        ...
        return (
            <>
                {this.state.loading ? <Loader isVisible={true} /> :
                    <div className='mb-2'>
                        <h2 className='mb-3 result'>Results of <strong>{searchTerm}</strong></h2>
